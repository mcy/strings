...
pub fn spec(_attr: TokenStream, item: TokenStream) -> TokenStream {
  // This is implemented as a decl macro, because that's easier to
  // understand and debug than proc macros. I hate proc macros so much.
  let span = Span::call_site();
  let macro_call: [TokenTree; 8] = [
    Punct::new(':', Spacing::Joint).into(),
    Punct::new(':', Spacing::Alone).into(),
    Ident::new("ilex", span).into(),
    Punct::new(':', Spacing::Joint).into(),
    Punct::new(':', Spacing::Alone).into(),
    Ident::new("__spec__", span).into(),
    Punct::new('!', Spacing::Alone).into(),
    Group::new(Delimiter::Brace, item).into(),
  ];

  macro_call.into_iter().collect()
}
